...
class InstallmentControllerIntegrationTest {
    ...
    void setUp() {
        ...
        testUser = User.create(new UserId(UUID.randomUUID()), new Email("test.installment@payhint.com"), "Password123!",
                "Test", "User");
    ...
    class AddInstallmentEndpoint {
        ...
        void shouldFailToAddInstallmentToAnotherUsersInvoice() throws Exception {
            ...
            Customer otherCustomer = customerRepository.save(Customer.create(new CustomerId(UUID.randomUUID()),
                    otherUser.getId(), "Other Co", new Email("other@co.com")));
        ...
        void shouldFailToAddInstallmentWithDuplicateDueDate() throws Exception {
            ...
            CreateInstallmentRequest request = new CreateInstallmentRequest(new BigDecimal("50.00"), dueDate);

            mockMvc.perform(post("/api/invoices/{invoiceId}/installments", testInvoice.getId().value())
                    .header("Authorization", "Bearer " + jwtToken).contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(request))).andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.detail")
                            .value(containsString("An installment with same due date already exists")));
        }
    }
    ...
    class UpdateInstallmentEndpoint {
        ...
        void shouldUpdateInstallmentSuccessfully() throws Exception {
            ...
            mockMvc.perform(put("/api/invoices/{invoiceId}/installments/{installmentId}", testInvoice.getId().value(),
        ...
        void shouldFailToUpdateInstallmentWithNegativeAmount() throws Exception {
            ...
            mockMvc.perform(put("/api/invoices/{invoiceId}/installments/{installmentId}", testInvoice.getId().value(),
                    installmentId.value()).header("Authorization", "Bearer " + jwtToken)
                            .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request)))
                    .andExpect(status().isBadRequest()).andExpect(jsonPath("$.errors", containsString("amountDue")));
        }
        ...
        void shouldFailToUpdateNonExistentInstallment() throws Exception {
            ...
            mockMvc.perform(put("/api/invoices/{invoiceId}/installments/{installmentId}", testInvoice.getId().value(),
                    UUID.randomUUID()).header("Authorization", "Bearer " + jwtToken)
                            .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request)))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.detail").value(containsString("Installment does not belong to invoice")));
        }
    ...
    class DeleteInstallmentEndpoint {
        ...
        void shouldDeleteInstallmentSuccessfully() throws Exception {
            mockMvc.perform(delete("/api/invoices/{invoiceId}/installments/{installmentId}",
                    testInvoice.getId().value(), installmentId.value()).header("Authorization", "Bearer " + jwtToken))
        ...
        void shouldFailToDeleteNonExistentInstallment() throws Exception {
            mockMvc.perform(delete("/api/invoices/{invoiceId}/installments/{installmentId}",
                    testInvoice.getId().value(), UUID.randomUUID()).header("Authorization", "Bearer " + jwtToken))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.detail").value(containsString("Installment does not belong to invoice")));
        }
        ...
        void shouldFailToDeleteInstallmentFromNonExistentInvoice() throws Exception {
            mockMvc.perform(delete("/api/invoices/{invoiceId}/installments/{installmentId}", UUID.randomUUID(),
                    installmentId.value()).header("Authorization", "Bearer " + jwtToken))
                    .andExpect(status().isNotFound());
        }
        ...
        void shouldFailToDeleteInstallmentFromOtherUserInvoice() throws Exception {
            ...
            mockMvc.perform(delete("/api/invoices/{invoiceId}/installments/{installmentId}",
                    otherInvoice.getId().value(), otherInstId.value()).header("Authorization", "Bearer " + jwtToken))
                    .andExpect(status().isNotFound());
        }
    }
}