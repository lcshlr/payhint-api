...
class CustomerControllerIntegrationTest {
    ...
    class GetInvoicesByCustomerIdEndpoint {
        ...
        void shouldRetrieveInvoicesForCustomer() throws Exception {
            ...
            InvoiceJpaEntity invoiceEntity = InvoiceJpaEntity.builder().invoiceReference("INV-2025-001").currency("EUR")
                    .build();
            invoiceEntity.setCustomer(customerEntity);

            InstallmentJpaEntity instEntity = InstallmentJpaEntity.builder().amountDue(BigDecimal.valueOf(100))
                    .dueDate(LocalDate.now().plusDays(30)).build();
            invoiceEntity.addInstallment(instEntity);

            PaymentJpaEntity paymentEntity = PaymentJpaEntity.builder().amount(BigDecimal.valueOf(50))
            ...
            mockMvc.perform(
                    get("/api/customers/{id}/invoices", customer.getId()).header("Authorization", "Bearer " + jwtToken))
        ...
        void shouldReturn403WhenAccessingAnotherUsersInvoices() throws Exception {
            ...
            var anotherCustomerEntity = customerSpringRepository.findById(anotherCustomer.getId().value()).get();
            InvoiceJpaEntity invoiceEntity = InvoiceJpaEntity.builder().invoiceReference("INV-2025-002").currency("EUR")
                    .build();
            invoiceEntity.setCustomer(anotherCustomerEntity);
            invoiceEntity = invoiceSpringRepository.save(invoiceEntity);

            mockMvc.perform(get("/api/customers/{id}/invoices", anotherCustomer.getId()).header("Authorization",
        ...
        void shouldReturn404WhenCustomerDoesNotExist() throws Exception {
            ...
            mockMvc.perform(
                    get("/api/customers/{id}/invoices", nonExistentId).header("Authorization", "Bearer " + jwtToken))
                    .andExpect(status().isNotFound()).andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.title").value("Resource Not Found"))
                    .andExpect(jsonPath("$.instance").value("/api/customers/" + nonExistentId + "/invoices"));
        }
        ...
        void shouldRejectInvalidUuidFormat() throws Exception {
            mockMvc.perform(
                    get("/api/customers/{id}/invoices", "invalid-uuid").header("Authorization", "Bearer " + jwtToken))
                    .andExpect(status().isBadRequest()).andExpect(jsonPath("$.status").value(400))
                    .andExpect(jsonPath("$.title").isNotEmpty());
        }
        ...
        void shouldRejectWithoutAuthenticationToken() throws Exception {
            ...
            mockMvc.perform(get("/api/customers/{id}/invoices", customerId)).andExpect(status().isForbidden());
        }
        ...
        void shouldRejectWithMalformedAuthorizationHeader() throws Exception {
            ...
            mockMvc.perform(get("/api/customers/{id}/invoices", customerId).header("Authorization", jwtToken))
                    .andExpect(status().isForbidden());
        }
    }
